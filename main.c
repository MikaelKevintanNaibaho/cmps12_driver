...
int main(void) {
  ...
  while (1) {
    ...
    // Print the data
    printf("Magnetometer: X=%u, Y=%u, Z=%u\n", all_data.magnetometer.x,
           all_data.magnetometer.y, all_data.magnetometer.z);
    printf("Accelerometer: X=%u, Y=%u, Z=%u\n", all_data.accelerometer.x,
           all_data.accelerometer.y, all_data.accelerometer.z);
    printf("Gyroscope: X=%u, Y=%u, Z=%u\n", all_data.gyro.x, all_data.gyro.y,
           all_data.gyro.z);
    printf("Orientation: Bearing=%u, Pitch=%d, Roll=%d\n",
           all_data.orientation.bearing, all_data.orientation.pitch,
           all_data.orientation.roll);
